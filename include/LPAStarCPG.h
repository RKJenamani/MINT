#ifndef LPA_STAR_CPG_
#define LPA_STAR_CPG_
...
class LPAStarCPG {
	...
	std::vector<Vertex> getNeighbors(Vertex &v, Graph &g)
	{
		...
		neighbors = cpg.getNeighborsImplicitCPG(v, g, left_map, right_map);
...
public:
	...
	void initLPA(Graph &composite_map, Graph & _left_map, Graph & _right_map, Vertex &start, Vertex &goal) 
	{
		...
		cpg.populateMaps(left_map,right_map,composite_map,mStartNode,mGoalNode);
	...
	std::vector<Vertex> computeShortestPath( Graph &g, double& costOut) 
	{
		...
		while (!isEmptyPQ() &&
			...
			// printPQ();
			// std::cout<<"Vertex pop'd : "<<(g[u].vertex_index)<<std::endl;
